...
function nextInvoiceNumber() {
    ...
    const maxNumber = invoices.reduce((max, invoice) => {
        const numericPart = parseInt(invoice.number.replace("INV-", ""), 10);
...
function setupInvoiceForm() {
    ...
    document
        .getElementById("openInvoiceForm")
        .addEventListener("click", () => {
    ...
    document
        .getElementById("closeInvoiceForm")
        .addEventListener("click", closeInvoiceForm);
    ...
    document
        ...
        .addEventListener("click", addInvoice);
}
...
function updateInvoicePreview() {
    ...
    document.getElementById("invoiceTaxPreview").textContent = money(tax);
    document.getElementById("invoiceTotalPreview").textContent = money(total);
}
...
function addInvoice() {
    ...
    if (!customerName) {
        showToast("Add a customer first", true);
        return;
    }

    if (!amount || amount <= 0) {
        showToast("Enter a valid invoice amount", true);
        return;
    }
    ...
    selectedRecord.invoices = newInvoice.number;
    renderDirectory("invoices", invoices);
    refreshCustomerIfOpen();
...
/* ============================================================
   SHIPMENTS — ADD FORM
   The delivery method drives everything: pickup costs
   nothing, an in-house (internal) delivery uses a manual
   cost, and a delivery company charges its internal or
   external rate automatically.
   ============================================================ */
...
function nextTrackingNumber() {
    ...
    const maxNumber = shipments.reduce((max, shipment) => {
        const numericPart = parseInt(shipment.tracking.replace("SHP-", ""), 10);
        return Number.isNaN(numericPart) ? max : Math.max(max, numericPart);
    }, 3000);

    return `SHP-${maxNumber + 1}`;
}
...
function setupShipmentForm() {
    ...
    document
        .getElementById("openShipmentForm")
        .addEventListener("click", () => {
            populateCustomerSelect("shipmentCustomer");
            populateCompanySelect("shipmentCompany");
            shipmentDraft.companyId = companies.length ? companies[0].id : null;
            setShipmentMethod("pickup");
            form.classList.remove("hidden");
        });

    document
        .getElementById("closeShipmentForm")
        .addEventListener("click", closeShipmentForm);
    ...
    document
        .querySelectorAll("#shipmentMethodSwitch button")
        .forEach(button => {
            button.addEventListener("click", () => {
                setShipmentMethod(button.dataset.method);
            });
        });

    document
        .querySelectorAll("#shipmentPriceTypeSwitch button")
        .forEach(button => {
            button.addEventListener("click", () => {
                setShipmentPriceType(button.dataset.priceType);
            });
        });

    document
        .getElementById("shipmentCompany")
        .addEventListener("change", function () {
            shipmentDraft.companyId = Number(this.value);
            updateShipmentPreview();
        });

    document
        .getElementById("shipmentInternalCost")
        .addEventListener("input", updateShipmentPreview);

    document
        .getElementById("saveShipment")
        .addEventListener("click", addShipment);
}


function setShipmentMethod(method) {

    shipmentDraft.method = method;
    ...
    document.getElementById("shipmentCompanyFields")
        .classList.toggle("hidden", method !== "company");

    document.getElementById("shipmentInternalFields")
        .classList.toggle("hidden", method !== "internal");

    updateShipmentPreview();
}


function setShipmentPriceType(priceType) {

    shipmentDraft.priceType = priceType;

    document
        .querySelectorAll("#shipmentPriceTypeSwitch button")
        .forEach(button => {
            button.classList.toggle("active", button.dataset.priceType === priceType);
        });
...
function calculateShipmentCost() {
    ...
    if (shipmentDraft.method === "pickup") {
        return 0;
    }

    if (shipmentDraft.method === "internal") {
        return Number(document.getElementById("shipmentInternalCost").value) || 0;
    }

    const company = companies.find(item => item.id === shipmentDraft.companyId);

    if (!company) {
        return 0;
    }

    return shipmentDraft.priceType === "external"
        ? company.externalPrice
        : company.internalPrice;
}


function updateShipmentPreview() {

    document.getElementById("shipmentCostPreview").textContent =
        money(calculateShipmentCost());
}
...
function closeShipmentForm() {
    ...
    document.getElementById("shipmentInternalCost").value = "";
}


function addShipment() {

    const customerName = document.getElementById("shipmentCustomer").value;
    ...
    let companyLabel = "Customer Pickup";

    if (shipmentDraft.method === "internal") {
        companyLabel = "Internal Delivery";
    } else if (shipmentDraft.method === "company") {

        const company = companies.find(item => item.id === shipmentDraft.companyId);

        if (!company) {
            showToast("Choose a delivery company", true);
            return;
        }
    ...
    selectedRecord.shipments = newShipment.tracking;
    renderDirectory("shipments", shipments);
    refreshCustomerIfOpen();
...
function refreshCustomerIfOpen() {

    if (selectedRecord.customers === undefined) {
        return;
    }

    selectRecord("customers", selectedRecord.customers, false);
}
...
function setupPlaceholderButtons() {
    ...
    document
        .querySelectorAll(
            ".page .primary-button:not(#openCustomerForm):not(#openInvoiceForm):not(#openShipmentForm)"
        )
        .forEach(button => {
